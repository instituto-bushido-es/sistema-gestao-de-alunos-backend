...
public class Graduacao implements Serializable {
     ...
     */
    public Falta adicionarFalta(String motivo, long data, Boolean justificada) {
        ...
        Falta novaFalta = new Falta(motivo, new Date(data), justificada);
    ...
    public int definirCargaHoraria() {
        ...
        long faltasNaoJustificadasCount = this.faltas.stream()
                .filter(falta -> falta.getJustificada() == null || !falta.getJustificada())
                .count();

        setCargaHoraria((int) ((quantidadeSemanasNaGraduacao * ValoresPadraoGraduacao.CARGA_HORARIA_SEMANAL_HORAS)
                - (faltasNaoJustificadasCount * 1)));
