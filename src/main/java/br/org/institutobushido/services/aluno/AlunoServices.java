...
public class AlunoServices implements AlunoServicesInterface {
    ...
    public List<AlunoDTOResponse> buscarAluno(String nome, String matricula, int pagina, int tamanho, String ordenarPor,
        ...
        throw new IllegalArgumentException("É necessário fornecer nome ou matrícula para busca");
    }
    ...
    public String adicionarFaltaDoAluno(String matricula, FaltaDTORequest falta, long dataFalta) {
        return adicionarFalta(matricula, falta.motivo(), dataFalta, falta.justificada());
    }

    private String adicionarFalta(String matricula, String motivo, long dataFalta, boolean justificada) {
        log.info("Entrou no adicionarFalta: ");
        Aluno aluno = this.encontrarAlunoPorMatricula(matricula);
        Falta novaFalta = aluno.getGraduacaoAtual().adicionarFalta(motivo, dataFalta, justificada);
        log.info("Metodo adicionarFalta falta: {}", novaFalta.getData());
    ...
    private void atualizarBancoDeDados(Aluno alunoEncontrado) {
        ...
        update.set("cpf", alunoEncontrado.getCpf());
        update.set("rg", alunoEncontrado.getRg());
        update.set("cartaoSus", alunoEncontrado.getCartaoSus());
    ...
    public void setupIndexes() {
        try {
            IndexOperations indexOps = mongoTemplate.indexOps(Aluno.class);

            org.springframework.data.mongodb.core.index.Index nomeIndex = new org.springframework.data.mongodb.core.index.Index()
                    .named("nome_index")
                    .on("nome", Sort.Direction.ASC);

            org.springframework.data.mongodb.core.index.Index compositeIndex = new org.springframework.data.mongodb.core.index.Index()
                    .named("nome_dataPreenchimento_index")
                    .on("nome", Sort.Direction.ASC)
                    .on("dataPreenchimento", Sort.Direction.ASC);

            indexOps.ensureIndex(nomeIndex);
            indexOps.ensureIndex(compositeIndex);

            System.out.println("Índices existentes na coleção Aluno:");
            indexOps.getIndexInfo().forEach(index -> {
                System.out.println("Nome do índice: " + index.getName());
                System.out.println("Campos do índice: " + index.getIndexFields());
                System.out.println("-------------------------");
            });

        } catch (Exception e) {
            System.err.println("Erro ao configurar índices: " + e.getMessage());
            e.printStackTrace();
        }
    }

    private List<AlunoDTOResponse> buscarAlunosPorNome(String nome, int page, int size, String sortOrder, String sortBy) {
        try {
            ...
            Criteria criteria = Criteria.where("nome")
                    .regex(".*" + Pattern.quote(nome) + ".*", "i");

            Query query = new Query(criteria).with(pageable);

            System.out.println("Query de busca: " + query.toString());

            List<Aluno> alunos = mongoTemplate.find(query, Aluno.class);

            return AlunoMapper.mapToListAlunoDTOResponse(alunos);

        } catch (Exception e) {
            System.err.println("Erro na busca de alunos por nome: " + e.getMessage());
