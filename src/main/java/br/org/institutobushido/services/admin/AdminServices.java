    ...
    protected String generateToken(Admin admin) {
        try {
            ...
            return JWT.create()
                    ...
                    .withExpiresAt(new Date(System.currentTimeMillis() + ONE_DAY_IN_MILIS))
                    .sign(algorithm);
...
/*
    ...
    public void criarAdminPadrao() {
        UserDetails adminEncontrado = adminRepositorio.findByEmail("admin@email.com");
        if (adminEncontrado == null) {
            SignUpDTORequest adminDTORequest = new SignUpDTORequest(
                    "admin", "admin@email.com", "123456", "admin", UserRole.ADMIN);
